...
class EstadisticaPaisController extends Controller{
    const SECTION = 8;
    protected $TextoModel, $Estadistica;
    
    const TEMPLATE = 25;
    const MAIN_ROUTE = "stat_link_estadistica_pais";
    const MAIN_ROUTE_ELITE = "stat_link_estadistica_pais_elite";
    ...
    public function indexAction(Request $request, $userType='general') {  
        ...
        $table = $userType != 'elite' ? 'sl_EstadisticaPais' : 'sl_EstadisticaPais_CE';
        $result = $this->Estadistica->getRegistered(
                $table, '', $idEdicion
        );

        foreach ($result as &$record) {
            $record = array("Pais" => $record['Pais' . strtoupper($content['lang'])], "Preregistro" => $record['Preregistro']);
        }

        $content['result'] = $result;  

        return $this->render('StatLinkEstadisticaBundle::pais.html.twig', array("content" => $content));
    }
}
