...
//        $this->configuracion = new VisitanteConfiguration();
    ...
    public function getDataTableAction(Request $request) {
        ...
        $result_count = $this->AsociadoModel->getCount('', $idEdicion);
        if (!$result_count['status']) {
            throw new \Exception($result_count['data'], 409);
        }
        $result_count = $result_count['data'];
        $result_list = $this->AsociadoModel->getListidVisitantes('', $idEdicion);
        $result_count_filter = array();
        if ($where != '') {
            $where = 'WHERE ' . $where;
            $result_list = $this->AsociadoModel->getListidVisitantes($where, $idEdicion);
            $result_count_filter = $this->AsociadoModel->getCount($where, $idEdicion);
        ...
        $result_query = $this->AsociadoModel->getVisitantes($where, $order, $post, $idEdicion);
                    ...
                    if ($idStatus == 2) {
                        if ($numeroEnvios > 0) {
                            if ($numeroDescargas > 0) {
                                ...
                                '<i class="material-icons download-record tooltipped blue-text" idVisitante="' . $idVisitante . '" data-position="left" data-tooltip="Descargar gafete">file_download</i>';
                                $data[$key][] = "";
                            } else {
                                $data[$key][] = '<i class="material-icons edit-record tooltipped" idVisitante="' . $idVisitante . '" data-position="left" data-tooltip="Editar">mode_edit</i>';
                                $data[$key][] = '<i class="material-icons send-record tooltipped blue-text" idVisitante="' . $idVisitante . '" data-position="left" data-tooltip="Enviar gafete">email</i>'.
                                '<i class="material-icons download-record tooltipped" idVisitante="' . $idVisitante . '" data-position="left" data-tooltip="Descargar gafete">file_download</i>';
                                $data[$key][] = "";
                            }
    ...
    public function downloadDigibadgeAction(Request $request) {
        ...
        $result_template_text = $this->AsociadoModel->getTexts($lang, 21);
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $accion = 1;
            $result_visitante = $this->AsociadoModel->updateDowloadLog($idEvento, $idEdicion, $idVisitante, $accion);
        }
    ...
    public function exportDataTableAction(Request $request) {
        ...
        $result_query = $this->AsociadoModel->getVisitantesExport($where, $order, $idEdicion);
        if (!$result_query['status']) {
            throw new \Exception($result_query['data'], 409);
        }
        $result_query = $result_query['data'];
...
//        }
    ...
    public function emailConfirmacionAction(Request $request) {
        ...
        $res = $this->get('ixpo_mailer')->send_email($general_text["ae_asuntoGafeteAntad2021"], $email, $body);
        /* Fin estructura envio de Email */
    ...
    public function sendDigibageeeeeeeAction(Request $request) {
        $session = $request->getSession();
        $lang = $session->get('lang');
        $idEvento = $session->get('edicion')["idEvento"];
        $idEdicion = $session->get('idEdicion');
        ...
        if ($request->getMethod() == 'POST') {
            $post = $request->request->all();
            $result_visitante = $this->AsociadoModel->getAsociadosGafete();
            if (!$result_visitante['status']) {
                throw new \Exception($result_visitante['data'], 409);
            }
            $idVisitante = array();
            ...
            foreach ($idVisitante as $key => $value) {
                ...
                $result_visitanteDG = $this->AsociadoModel->getVisitanteDG($idVisitante);
