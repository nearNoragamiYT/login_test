...
class VisitanteController extends Controller
{
    ...
    public function getToDataTableAction(Request $request)
    {
        ...
        $params["where"][] = array("name" => 'vis."RegistroMultiple"', "operator" => "=", "value" => '0', "type" => \PDO::PARAM_STR, "clause" => "AND");
    ...
    public function sendGafeteDigitalAction(Request $request)
    {
        ...
        $result_general_text = $this->VisitanteModel->getTexts($lang);
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($result_visitante['status'] == 1) {
                ...
                if ($result) {
                    ...
                    $accion = 2;
                    $result_visitante = $this->VisitanteModel->updateDowloadLog($idEvento, $idEdicion, $idVisitante, $accion);
                }
            } else {
    ...
    public function downloadDigibadgeAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            $post = $request->request->all();
            $idVisitante = $post['idVisitante'];
            $result_visitante = $this->VisitanteModel->getVisitanteDG($idVisitante);
