...
namespace Visitante\ComprasBundle\Controller;
...
class ComprasController extends Controller {
    ...
    protected $TextoModel, $ComprasModel, $configuracion;

    const TEMPLATE = 9;
    const MAIN_ROUTE = "compras";
...
//        $content['TipoCambio'] = $result_tipoCambio['data'][0];
        ...
        $session->set('columns_comp', $content["Visitante_table_columns"]);
        $content['currentRoute'] = $request->get('_route');
        $content['tabPermission'] = json_decode($this->ComprasModel->tabsPermission($user), true);
        /*Colocamos la ruta a utilizar en el detalle de la compra*/
        $session->set("purchase_origin", "compras");
        return $this->render('VisitanteComprasBundle:Compras:lista_compras.html.twig', array('content' => $content));
    }
    ...
    public function getToDataTableAction(Request $request) {
        ...
        $idEvento = $session->get('edicion')["idEvento"];
        $idEdicion = $session->get('idEdicion');
        $session->set("edicion_comp", $idEdicion);
        $session->remove('seting-dt_comp');
        ...
        if (!$result_count["status"]) {
            ...
            $session->set('qry_count_comp', $result_count["count"]);
        }
        ...
        if (array_key_exists("where", $params) && COUNT($params["where"]) > 0) {
            ...
            if (!$result_filtered_count["status"]) {
                ...
                $session->set('count_filtered_comp', $result_filtered_count["count"]);
            }
            $records_filtered = $result_filtered_count["data"][0]["total"];
        } else {
            $session->remove('count_filtered_comp');
            $params["where"][] = Array("name" => 'vise."idEdicion"', "operator" => "=", "value" => $idEdicion, "type" => \PDO::PARAM_STR);
            $params["where"][] = Array("name" => 'vis."Prueba"', "operator" => "=", "value" => "0", "type" => \PDO::PARAM_STR, "clause" => "AND");
        }
        ...
        if (!$result_query["status"]) {
            ...
            $session->set('data_qry_comp', $result_query["data_qry"]);
        }
    ...
    public function getToDataTableFilterAction(Request $request) {
        ...
        $idEvento = $session->get('edicion')["idEvento"];
        $idEdicion = $session->get('idEdicion');
        $edicion_visit = $session->get("edicion_comp");
        $text = $this->ComprasModel->getTexts($lang, self::TEMPLATE);

        if ($idEdicion != $edicion_visit) {
            $session->remove('seting-dt_comp');
            $session->remove('qry_count_comp');
            $session->remove('count_filtered_comp');
            $session->remove('data_qry_comp');
        }
        $session->set("edicion_comp", $idEdicion);
        ...
        $post = $request->request->all();
        if ($session->has('qry_count_comp')) {
            $qry_count = $session->get('qry_count_comp');
        ...
        if ($session->has("count_filtered_comp")) {
            $count_filtered = $session->get('count_filtered_comp');
        ...
        if ($session->has("data_qry_comp")) {
            $data_qry = $session->get('data_qry_comp');
    ...
    public function exportComprasDataAction(Request $request) {
        $this->ComprasModel = new ComprasModel($this->container);
        $session = $request->getSession();
        $lang = $session->get("lang");
        $idEvento = $session->get('edicion')["idEvento"];
    ...
    public function setSessionDataAction(Request $request) {
        $session = $request->getSession();
        $post = $request->request->all();
        $session->set('seting-dt_comp', $post);
        return $this->jsonResponse($post);
    }

    public function getSessionDataAction(Request $request) {
        $session = $request->getSession();
        $data = array("seting" => $session->get('seting-dt_comp'), "param" => $session->get('data_qry_comp')["params"]["where"], "columns" => $session->get('columns_comp'));
    ...
    public function getCompraDataAction(Request $request, $idVisitante, $idCompra) {
        ...
        $purchaseOrigin = $session->get("purchase_origin");
        /* Verificamos si tiene permiso en el modulo seleccionado */
        $breadcrumb = $this->get("verificador_modulo")->rastrearBreadcrumbs($request, $purchaseOrigin);
        if (!$breadcrumb) {
            $session->getFlashBag()->add('warning', $content['general_text']['sas_moduloNoDisponible']);
            return $this->redirectToRoute('show_dashboard_edicion', array('idEdicion' => $session->get('idEdicion'), 'lang' => $lang));
        }
