...
class SQLModelFactura {

    public $PGModel;
    private $schema;

    public function __construct($db_data = 'PGSQL_SAS_FACTURACION') {
    ...
    public function prepareQueryConfig($config) {
        $qry = "";
        if (COUNT($config) > 0) {
            if ($config['order_fields']) {
                $qry .= ' ORDER BY ' . $config['order_fields'] . ' ';
                $qry .= ($config['order'] != null) ? $config['order'] : ' ASC ';
            }
        }
        if (isset($config['limit']) && isset($config['offset'])) {
            $qry .= ' LIMIT ' . $config['limit'] . ' OFFSET ' . $config['offset'];
        }
        return $qry;
    }
