...
namespace ShowDashboard\LT\ReportesBundle\Model;
...
class LectorasModel extends DashboardModel {
    ...
    public function getCountExhibtors($args = Array(), $conditions = Array()) {
        ...
        $qry .= '     FROM "LECTORAS"."vw_sas_ObtenerExpositores"';
    ...
    public function getSoliciudLectorasReporte($params) {
        ...
        $result = $this->SQLModel->selectFromTable("vw_sas_ObtenerSolicitudLectoras", $fields, $params);
        if (isset($result['status']) && $result['status'] == 1) {
            $data = Array();
            if (COUNT($result['data']) > 0) {
                foreach ($result['data'] as $value) {
    ...
    public function getReporteSeguimiento($params) {
        ...
        $qry .= ' FROM "LECTORAS"."vw_sas_ReporteSeguimientoLectoras_nuevo" ';
        $qry .= ' WHERE "idEdicion" = ' . $params["idEdicion"] . ' AND "idEvento" = ' . $params["idEvento"];
    ...
    public function getReporteSeguimientoDetalle($idEdicion, $idEvento){
        $qry = ' SELECT "idEmpresa", "Servicio", "Cantidad", "Precio"';
        $qry .= ' FROM "LECTORAS"."vw_sas_ReporteSeguimientoLectoras_nuevo" ';
    ...
    public function getReporteGlobal($params) {
        $qry = ' SELECT ' . $this->getGlobalFields();
        $qry .= ' FROM "LECTORAS"."vw_lectoras_GlobalReport" ';
        $qry .= ' WHERE "idEdicion" = ' . $params["idEdicion"] . ' AND "idEvento" = ' . $params["idEvento"];
    ...
    public function getDetalleGR($id) {     // Consulta el detalle del global report
        $qry = ' SELECT ' . $this->getGlobalFields();
        $qry .= ' FROM "LECTORAS"."vw_lectoras_GlobalReport" ';
        /* $qry .= ' FROM "LECTORAS"."vw_sas_GlobalReport" '; */
        $qry .= ' WHERE "idEmpresa" = ' . $id;
        $result = $this->SQLModel->executeQuery($qry);
    ...
    protected function getGlobalFields() {
        ...
        $fields .= ' "idEmpresa",';
        $fields .= ' "DC_NombreComercial",';
        $fields .= ' CASE WHEN "Contacto_Forma_ApellidoPaterno" IS NULL THEN ';
        $fields .= ' "Contacto_Forma_Nombre" ';
        $fields .= 'ELSE';
        $fields .= ' ("Contacto_Forma_Nombre" ';
        $fields .= " || ' ' || ";
        $fields .= '"Contacto_Forma_ApellidoPaterno"';
        $fields .= ') END AS NombreContactoContratacion,';
        $fields .= ' "Contacto_Forma_Email",';
        $fields .= ' "Contacto_Forma_Celular",';
        $fields .= ' "CelularResponsiva",';
        $fields .= ' "StandNumber",';
        $fields .= ' "APP, SIN EQUIPO SOLICITADAS" AS app_sin_equipo_solicitadas,';
        $fields .= ' "APP + EQUIPO (CELULAR) SOLICITADAS" AS app_mas_equipo_celular_solicitadas,';
        $fields .= ' "PAQUETE A DE 3 A 5 APP´s SOLICITADAS" AS paquete_a_de_3_a_5_apps_solicitadas,';
        $fields .= ' "PAQUETE B DE 6 A 10 APP´s SOLICITADAS" AS paquete_b_de_6_a_10_apps_solicitadas,';
        $fields .= ' "MINI SCAN WIRELESS SOLICITADAS" AS mini_scan_wireless_solicitadas,';
        $fields .= ' "App ASIGNADAS" AS app_asignadas,'; 
        $fields .= ' "App + equipo ASIGNADAS" AS app_mas_equipo_asignada,';
        $fields .= ' "Mini Scan Wireles ASIGNADAS" AS mini_scan_wireless_asignadas';
