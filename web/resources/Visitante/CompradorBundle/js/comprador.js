...
$(document).ready(function () {
    ...
    if(user['idUsuario'] != 1 && user['idUsuario'] != 35 && user['idUsuario'] != 58){
        columnas.pop();
        columnas.pop();
    }
        ...
        success: function (response) {
            if (!response['status']) {
                return;
                        ...
                        location.reload();
            show_toast("success", "Enviado Correctamente");
        },
        error: function (request, status, error) {
            alert('ERROR SEND');
        }
...
$(document).on("click",'.download-record',function(){
    ...
    $.ajax({
        ...
        success: function (response, status, xhr) {
            ...
            setTimeout(function(){ URL.revokeObjectURL(downloadUrl); },1000); //Limpieza
            location.reload();
